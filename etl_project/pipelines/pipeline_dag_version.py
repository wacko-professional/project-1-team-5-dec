...
if __name__ == "__main__":
    ...
    try:
        ...
        pipeline_logger.log_to_file(message=f"START: Extracting rates data for the past {day_count} days")
        ...
        for date_requested in (last_update + timedelta(days=n+1) for n in range(day_count)):
         ...
         # Transform and Load
        pipeline_logger.log_to_file(message=f"END: Extracted rates data for the past {day_count} days")

        pipeline_logger.log_to_file(message=f"START: Initializing database parameters")
        ...
        pipeline_logger.log_to_file(message=f"END: Initialized database parameters")

        pipeline_logger.log_to_file(message=f"START: Initializing SqlTransform objects and DAGs")

        transform_environment = Environment(loader=FileSystemLoader("etl_project/sql/transform"))
        rates_serving = SqlTransform(engine=staging_postgresql_client.engine, environment=transform_environment, table_name="rates_serving")
        moving_averages = SqlTransform(engine=staging_postgresql_client.engine, environment=transform_environment, table_name="moving_averages")
        relative_strengths = SqlTransform(engine=staging_postgresql_client.engine, environment=transform_environment, table_name="relative_strengths")
        volatility = SqlTransform(engine=staging_postgresql_client.engine, environment=transform_environment, table_name="volatility")
        year_over_year = SqlTransform(engine=staging_postgresql_client.engine, environment=transform_environment, table_name="year_over_year")
        month_to_date = SqlTransform(engine=staging_postgresql_client.engine, environment=transform_environment, table_name="month_to_date")

        dag = TopologicalSorter()
        dag.add(rates_serving)
        dag.add(moving_averages, rates_serving)
        dag.add(relative_strengths, rates_serving)
        dag.add(volatility, rates_serving)
        dag.add(year_over_year, rates_serving)
        dag.add(month_to_date, rates_serving)

        dag_tuple = tuple(dag.static_order())
        dag_reversed = reversed(dag_tuple)

        pipeline_logger.log_to_file(message=f"END: Initialized SqlTransform objects and DAGs")

        # add to SqlTransform Class in the future as delete table
        pipeline_logger.log_to_file(message=f"START: Starting the whole transform process")

        for node in dag_reversed:
            # table_name = node.table_name
            # node.engine.execute(f"drop table if exists {table_name};")
            node.drop_table_if_exists()
            pipeline_logger.log_to_file(message=f"Deleting table {node.table_name} successful")


        for node in dag_tuple:

            node.create_table_as() # if the for loop in dag_reversed will be used, the drop table if exists in exec_sql can be removed in SqlTransform.create_table_as method
            
            pipeline_logger.log_to_file(message=f"Transforming and loading table {node.table_name} successful")
            # table_name = node.table_name # add to SqlTransform Class in the future as Alter Table method -> gets table_name of node
            # alter_template = node.environment.get_template(f"{table_name}_alter.sql") # add to SqlTransform Class in the future as Alter Table method -> gets the template from the environment of the node
            # node.engine.execute(alter_template.render()) # add to SqlTransform Class in the future as Alter Table method -> gets the engine from the node and executes the query in the template

            node.alter_table()

            pipeline_logger.log_to_file(message=f"Constraints added to {node.table_name}")
        
        pipeline_logger.log_to_file(message=f"END: Transform process completed successfully")
        ...
        pipeline_logger.log_to_file(message="END: Pipeline run successful")
        metadata_logging.log(status=MetaDataLoggingStatus.RUN_SUCCESS, logs=pipeline_logger.get_logs())

    except Exception as e:
        pipeline_logger.log_to_file(message=f"Pipeline failed with exception {e}")
